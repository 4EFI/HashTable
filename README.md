...
Хеш-таблицу заполним, опираясь на произведение Уильяма Шекспира "Гамлет, принц Датский" в оригинале, в котором более 31000 слов. Размер таблицы возьмем равным 1001 элементов (берем простое число, спросите почему? это к математикам :)) ), чтобы средняя заселенность была 10-20 элементов.
___
...
<details>
<summary> Диаграмма при большем размере хеш-таблицы </summary>

Также стоить рассмотреть данную хеш-функцию, при достаточно большом размере таблицы (на примере 5000 элементов). 

<p style="text-align: center"><img src=res/bar_sum_ascii_5000.png width="550px"/></p>

Из диаграммы видно, что начиная с 1500 элемента таблица пустая. Она нам лишь показалась хорошей, но стоит нам увеличить количество данных, как эффективность функции теряется. 
...
| Оптимизация | Число выполненных инструкций | Коэффициент ускорения | 
|-------------|-----------------------|-----------------------|
|    Нет      |      1 833 595        |          1.0          |

<p style="text-align: center"><img src=res/opt_graph_0.png width="700px"/></p>

В данном графе 0x000... ни что иное как вызов функции strcmp, не стоит пугаться :))  
...
# <p style="text-align: center"><img src=res/opt.png width="700px"/></p> 
...
P.S. Точки и есть нулевые символы. 
